...
tox = 2e-7; % The thickness of SiO2 in the capacitor [cm] 
...
% Additional steps to compute charge density and electric field
VG = V_G; % Assuming VG is the gate voltage vector
...
for i = 1:length(VG)
    ...
    psi_solution = ode45(@(x, psi) F(psi, interp1(VG, VG, x)), [min(VG), max(VG)], 0);
...
capacitance = abs(gradient(Qs, V_G));
% Plot capacitance vs. gate voltage
figure;
plot(V_G, capacitance);
xlabel('Gate Voltage (V)');
ylabel('Capacitance (F/m^2)');
title('Capacitance vs. Gate Voltage');
